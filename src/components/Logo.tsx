/**
 * SafeSwap brand marks — vectors taken directly from the official
 * Visual Identity guide (shield + bi-directional arrows forming an "S").
 */
...
export function ShieldMark({
  ...
  return (
    ...
    >
      <path
        fill="#01a78f"
        className={animated ? "origin-center [animation:float_7s_ease-in-out_infinite]" : ""}
...
export function Wordmark({ className = "" }: { className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-2.5 font-sans ${className}`}
    >
      <ShieldMark className="h-7 w-auto" />
      <span className="text-[1.35rem] font-medium leading-none tracking-tight text-mint-pale">
